...
__global__ void multi_head_attention_kernel(int32_t pos, int32_t seq_len, float* query_ptr,
                                            float* score_ptr, float* output_ptr, float* key_cache,
  ...
  float* query_head_ptr = query_ptr + head_idx * head_size;
  float* score_head_ptr = score_ptr + head_idx * seq_len;
  int head_offset = (head_idx / kv_mul) * head_size;

  for (int t = threadIdx.x; t <= pos; t += blockDim.x) {
    /*
    the overall kv-cache mem structure : [layer][time_steps][kv_heads][head_size]
    key_cache : base addr
    layer_offset : transformer layers
    seq_len : model max token len
    t * kv_dim : locate the cur timestep
    head_offset : locate the cur head pos
    */

    float* key_head_ptr = key_cache + layer_offset + t * kv_dim + head_offset;
    float score = 0;
#pragma unroll
    for (int i = 0; i < head_size; i += 4) {
      float4 key_head_float4 = *(float4*)(key_head_ptr + i);
      float4 query_head_float4 = *(float4*)(query_head_ptr + i);
    ...
    score *= scale;
    score_head_ptr[t] = score; 
  }

  __syncthreads();
  softmax_gpu(score_head_ptr, pos+1);
  __syncthreads();

  float* output_head_ptr = output_ptr + head_idx * head_size;
  for (int i = threadIdx.x; i < head_size; i += blockDim.x) {
    float value = 0;
#pragma unroll
    for (int t = 0; t <= pos; t++) {
      float* value_head_ptr = value_cache + layer_offset + t*kv_dim + head_offset; 
      float score = score_head_ptr[t];
      value += score * value_head_ptr[i];
    }
